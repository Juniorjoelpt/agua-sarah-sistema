...
// Mesmo timbrado e estilo do OrcamentoPdfService principal - documento
// formal pra entregar a uma empresa terceira.
@Service
public class OrcamentoTerceiroPdfService {
    ...
    public byte[] gerar(OrcamentoTerceiro orcamento) {
        try {
            ...
            Paragraph titulo = new Paragraph("Orçamento (Terceiros) Nº " + orcamento.getId(), FONTE_TITULO);
            ...
            cellCliente.addElement(new Paragraph(orcamento.getCliente().getNome(), FONTE_CELULA_NEGRITO));
            if (orcamento.getCliente().getTelefone() != null && !orcamento.getCliente().getTelefone().isBlank()) {
                cellCliente.addElement(new Paragraph(orcamento.getCliente().getTelefone(), FONTE_CELULA_MUTED));
            }
            ...
            int linha = 0;
            for (ItemOrcamentoTerceiro item : orcamento.getItens()) {
            ...
            totalCell.addElement(new Paragraph("Valor total", FONTE_SUBTITULO));
            totalCell.addElement(new Paragraph(FMT_MOEDA.format(orcamento.getValorTotal()), FONTE_TOTAL));
            ...
            throw new IllegalStateException("Erro ao gerar PDF do orçamento de terceiros", e);
        }
    }
