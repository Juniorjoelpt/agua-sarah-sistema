...
export default function ContasReceber() {
  ...
  async function carregar() {
    ...
    try {
      const [c, cli] = await Promise.all([
        contasReceberApi.listar(undefined, somenteEmAberto),
        clientesApi.listar(),
      ]);
      setContas(c);
      setClientes(cli);
  ...
  async function confirmarPagamento() {
    ...
    setErro(null);
    try {
      await contasReceberApi.registrarPagamento(contaPagamento.id, {
  ...
  return (
    <div>
      <SectionHeaderWithAction title="Contas a Receber" subtitle="Vendas fiado e dívidas de clientes" actionLabel="+ Nova conta" onAction={abrirNovaConta} />
        ...
        {contaPagamento && (
          <>
            ...
            <div className="flex gap-2 mt-5">
              <button
                type="button"
                disabled={salvandoPagamento || totalPagamentoInput <= 0 || totalPagamentoInput > saldoDevedorPagamento + 0.01}
