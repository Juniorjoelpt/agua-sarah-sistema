...
import { despesasTerceirosApi } from '../../api/terceiros/despesas';
...
export default function DespesasTerceiros() {
  ...
  async function carregar() {
    setCarregando(true);
    try {
      setDespesas(await despesasTerceirosApi.listar());
    } catch (e) {
      setErro(e.message);
    }
    setCarregando(false);
  ...
  async function salvar() {
    ...
    setErro(null);
    try {
      let caixaTerceiroId = null;
      if (vincularCaixa === 'SIM') {
        try {
          const caixa = await caixaTerceirosApi.atual();
          caixaTerceiroId = caixa?.id ?? null;
        } catch {
          caixaTerceiroId = null;
        }
      }
      await despesasTerceirosApi.criar({ ...form, valor: Number(form.valor), caixaTerceiroId });
  ...
  return (
    <div>
      <SectionHeaderWithAction title="Despesas" subtitle="Despesas isoladas do módulo de terceiros" actionLabel="+ Nova despesa" onAction={abrirNovo} />
      ...
      <Modal open={modalAberto} title="Nova despesa" onClose={() => setModalAberto(false)}>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <Field label="Sai do caixa de terceiros do dia?">
        ...
        <div className="grid grid-cols-3 gap-4">
          {despesas.map((d) => (
            <Card key={d.id} hover>
              ...
              {d.contaPagar && <Badge tone="neutral">Pagamento de conta a pagar</Badge>}
            </Card>
          ))}
        </div>
      )}
