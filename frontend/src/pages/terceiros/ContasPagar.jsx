...
const NOVA_CONTA_VAZIA = { descricao: '', categoria: 'OUTROS', valorOriginal: '', dataVencimento: '', observacao: '' };
...
function situacao(conta) {
  ...
  return { label: conta.status === 'PARCIAL' ? 'Parcial' : 'Em aberto', tone: 'neutral' };
}

export default function ContasPagarTerceiros() {
  ...
  async function carregar() {
    setCarregando(true);
    try {
      setContas(await contasPagarTerceirosApi.listar(somenteEmAberto));
      try {
        setCaixaAberto(await caixaTerceirosApi.atual());
      } catch {
        setCaixaAberto(null);
      }
    } catch (e) {
  ...
  const totalVencidas = contas.filter((c) => c.status !== 'PAGA' && c.dataVencimento < hoje()).reduce((s, c) => s + (c.valorOriginal - c.valorPago), 0);
  ...
  async function salvarNovaConta() {
    ...
    setErro(null);
    try {
      await contasPagarTerceirosApi.criar({
  ...
  async function confirmarPagamento() {
    ...
    setErro(null);
    try {
      await contasPagarTerceirosApi.registrarPagamento(contaPagamento.id, {
        valor: Number(valorPagamentoInput || 0),
        caixaTerceiroId: saiuDoCaixa && caixaAberto?.id ? caixaAberto.id : null,
      });
  ...
  async function confirmarExclusao() {
    ...
    setErro(null);
    try {
      await contasPagarTerceirosApi.excluir(paraExcluir.id);
      setParaExcluir(null);
      await carregar();
    } catch (e) {
      setErro(e.message);
  ...
  return (
    <div>
      <SectionHeaderWithAction title="Contas a Pagar" subtitle="Obrigações do módulo de terceiros, com vencimento" actionLabel="+ Nova conta" onAction={abrirNovaConta} />
      ...
      <Modal open={modalNovaConta} title="Nova conta a pagar" onClose={() => setModalNovaConta(false)} maxWidth={480}>
        ...
        <Field label="Descrição"><TextInput placeholder="Ex: Manutenção do caminhão" value={novaConta.descricao} onChange={(e) => setNovaConta({ ...novaConta, descricao: e.target.value })} /></Field>
        <div className="mt-3">
        ...
        {contaPagamento && (
          <>
            <div className="rounded-lg p-3 mb-4" style={{ background: C.bg }}>
              <div className="flex justify-between" style={{ fontSize: 13 }}><span style={{ color: C.textMuted }}>Valor original</span><span style={{ color: C.textDark }}>{moeda(contaPagamento.valorOriginal)}</span></div>
            ...
            <Field label="Valor a pagar agora"><TextInput type="number" step="0.01" value={valorPagamentoInput} onChange={(e) => setValorPagamentoInput(e.target.value)} /></Field>
            ...
            {caixaAberto?.id ? (
              <label className="flex items-center gap-2 mt-4" style={{ fontSize: 13, color: C.textDark }}>
                ...
                Saiu do caixa de terceiros aberto hoje
              </label>
            ) : (
              <div className="flex items-center gap-2 mt-4 px-3 py-2 rounded" style={{ background: C.bg, color: C.textMuted, fontSize: 12 }}>
                <AlertTriangle size={14} /> Nenhum caixa de terceiros aberto — não vai debitar de caixa nenhum.
              </div>
            )}

            <div className="flex gap-2 mt-5">
              <button type="button" disabled={salvandoPagamento || Number(valorPagamentoInput || 0) <= 0} onClick={confirmarPagamento} className="px-4 py-2 rounded text-sm font-medium disabled:opacity-60" style={{ background: C.red, color: '#fff' }}>
              ...
              <Card key={conta.id}>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3 min-w-0">
                    <Avatar name={conta.descricao} color={C.blue} />
                    <div className="min-w-0">
                      <div style={{ fontSize: 14, fontWeight: 600, color: C.textDark }} className="truncate">{conta.descricao}</div>
                      <div style={{ fontSize: 12, color: C.textMuted }} className="truncate">{CATEGORIA_LABEL[conta.categoria]} · vence em {dataCurta(conta.dataVencimento)}</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-4 flex-shrink-0">
                    ...
                    {conta.status !== 'PAGA' && (
                      <button onClick={() => abrirPagamento(conta)} className="px-3 py-1.5 rounded text-xs font-medium" style={{ background: C.red, color: '#fff' }}>Pagar</button>
                    )}
                    {conta.valorPago === 0 && (
                      <button onClick={() => setParaExcluir(conta)} className="px-2 py-1.5 rounded text-xs" style={{ color: '#B3261E' }}>Excluir</button>
                    )}
                  </div>
                </div>
              </Card>
